...
Decimal.setMethod(function isGreaterThan(other_value) {
	...
	let other_decimal = this.ensure(other_value),
	    this_positive = this.isPositive(),
	    other_positive = other_decimal.isPositive();

	if (this_positive && !other_positive) {
		return true;
	} else if (!this_positive && other_positive) {
		return false;
	} else if (this_positive && other_positive) {
...
Decimal.setMethod(function divide(value) {
	...
	// Use Decimal.ensure instead of this.ensure to preserve divisor precision
	// (FixedDecimal.ensure would round to this's scale, potentially causing divide by zero)
	let other_decimal = Decimal.ensure(value);
