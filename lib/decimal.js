 ...
 * @version  0.8.14
 *
 * @param    {BigInt}   unified_result
 * @param    {Number}   unified_fractional_length
 * @param    {Number}   target_fractional_length
 *
 * @return   {Decimal}
 */
Decimal.setMethod(function returnArithmeticResult(unified_result, unified_fractional_length, target_fractional_length) {
	let result = Object.create(Decimal.prototype);
	return result._applyArithmeticResult(unified_result, unified_fractional_length, target_fractional_length);
});
 ...
 * @param    {Number}   unified_fractional_length  The fractional length used to scale
 * @param    {Number}   target_fractional_length   The fractional length to use for the result
 *
 * @return   {Decimal}
 */
Decimal.setMethod(function _applyArithmeticResult(unified_result, unified_fractional_length, target_fractional_length) {

	let scale = 10n ** BigInt(unified_fractional_length);

	let integral = unified_result / scale;
	let fractional = unified_result % scale;

	if (target_fractional_length == null) {
		target_fractional_length = this.calculateTargetFractionalLength(fractional, unified_fractional_length);
	}

	if (unified_fractional_length == target_fractional_length) {
		target_fractional_length = unified_fractional_length;
	} else {
		let reduction_factor = 10n ** BigInt(unified_fractional_length - target_fractional_length);
		fractional = fractional / reduction_factor;
	}

	this[FRACTIONAL_LENGTH] = target_fractional_length;
	this[FRACTIONAL_LENGTH_BI] = BigInt(target_fractional_length);
 ...
 * @return   {Number}
 */
Decimal.setMethod(function calculateTargetFractionalLength(fraction, unified_fractional_length) {
	...
	if (zeroes >= unified_fractional_length) {
		return 0;
	}

	// Otherwise, we can return the difference
	return unified_fractional_length - zeroes;
});
...
/**
 * Multiply the given value with this value and return the new result.
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.8.14
 * @version  0.8.14
...
Decimal.setMethod(function multiply(value) {
	...
	let sum_fractional_length = this[FRACTIONAL_LENGTH] + other_decimal[FRACTIONAL_LENGTH];

	// Get the scaled integral and fractional parts of this value
	let this_scaled = this.toScaledBigInt(this[FRACTIONAL_LENGTH]);
	let other_scaled = other_decimal.toScaledBigInt(other_decimal[FRACTIONAL_LENGTH]);

	// Multiply the scaled values
	let scaled_result = this_scaled * other_scaled;

	return this.returnArithmeticResult(scaled_result, sum_fractional_length);
});
