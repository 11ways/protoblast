module.exports = function serverFunctions(Blast, extras) {
	...
	let temp_tracker_created,
	    temp_dir,
	    client_file_cache = new Map(),
	    temp_dir_watcher = null,
	    client_file_to_cache_id = new Map();

	/**
	 * Ensure the temp directory watcher is set up (only once)
	 *
	 * @author    Jelle De Loecker <jelle@elevenways.be>
	 * @since     0.9.6
	 * @version   0.9.6
	 */
	function ensureTempDirWatcher() {
		if (temp_dir_watcher) {
			return;
		}

		try {
			temp_dir_watcher = fs.watch(Blast.temp_dir, (eventType, filename) => {
				// Guard: if temp_dir is null, cleanup is in progress - ignore events
				if (!temp_dir) {
					return;
				}
				...
				if (eventType === 'rename') {
					...
					fs.access(Blast.temp_dir, fs.constants.F_OK, (dirErr) => {
						if (dirErr) {
							// Directory was deleted! Reset everything
							temp_dir = null;
							temp_dir_watcher.close();
							temp_dir_watcher = null;
							client_file_cache = new Map();
							client_file_to_cache_id.clear();
							return;
						}

						// Directory exists, check if a specific file was deleted
						if (filename) {
							let filePath = libpath.join(Blast.temp_dir, filename);
							let cacheIds = client_file_to_cache_id.get(filePath);
								...
								fs.access(filePath, fs.constants.F_OK, (err) => {
									if (err) {
										...
										for (let cacheId of cacheIds) {
											client_file_cache.delete(cacheId);
										}
										client_file_to_cache_id.delete(filePath);
									}
								});
							}
						}
	...
	Blast.getClientPath = function getClientPath(options) {
		...
		if (client_file_cache.has(id) && !refresh) {
			return client_file_cache.get(id);
		}
		...
		if (client_file_cache.has(compose_id) && !refresh) {
			client_file_cache.set(id, client_file_cache.get(compose_id));
			return client_file_cache.get(id);
		}
		...
		client_file_cache.set(id, pledge);
		client_file_cache.set(compose_id, pledge);

		Fn.parallel(tasks, async (err, files) => {

			if (err) {
				return client_file_cache.get(id).reject(err);
			}
		...
		async function handleFiles(files) {
			...
			fs.writeFile(temp_file.fd, template, err => {
				...
				client_file_cache.get(id).resolve(temp_file.path);

				// Register the file path for cache invalidation via directory watcher
				client_file_to_cache_id.set(temp_file.path, [id, compose_id]);
				ensureTempDirWatcher();
			});
		}
	...
	Blast.cleanupTempPaths = function cleanupTempPaths() {
		...
		// Close the watcher first to prevent race conditions
		if (temp_dir_watcher) {
			temp_dir_watcher.close();
			temp_dir_watcher = null;
		}
