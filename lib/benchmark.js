module.exports = function BlastBenchmark(Blast, Collection) {
	...
	function doSyncBench(fn, callback) {
		...
		if (callback) {
			...
			console.log('Benchmark ' + name + 'did ' + Blast.Bound.Number.humanize(result.ops) + '/s (' + Blast.Bound.Number.humanize(result.iterations) + ' iterations)');
		}
