...
Blast.getSignatureWrapperMethod = function getSignatureWrapperMethod(context, name) {
	...
	if (!wrapper_method) {
		...
		wrapper_method = Fn.create(name, function _doCorrectTypeMethod(...args) {
			...
			if (types_by_length.has_before_instrumentation) {
				let i;

				for (i = 0; i < types_by_length.before.length; i++) {
					types_by_length.before[i](this, args);
				}
			}

			if (!signature) {
				if (wrapper_method.super) {
					result = wrapper_method.super.apply(this, args);
				} else if (current_method) {
					result = current_method.apply(this, args);
				} else {
					throw new TypeError('Failed to find "' + name + '" method matching signature `' + argsToSignatureString(args) + '`');
				}
			} else {
				result = signature.execute(this, args);
			}

			if (types_by_length.has_after_instrumentation) {
				let i;

				for (i = 0; i < types_by_length.after.length; i++) {
					types_by_length.after[i](this, args, result);
				}
			}
...
Blast.addMethodInstrumentation = function addMethodInstrumentation(context, method_name, before, after) {
	...
	if (before) {
		types_by_length.has_before_instrumentation = true;
		types_by_length.before.push(before);
	}

	if (after) {
		types_by_length.has_after_instrumentation = true;
		types_by_length.after.push(after);
	}
};
