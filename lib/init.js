...
function BlastInit(modifyPrototype) {
			...
			for (i = 0; i < files.length; i++) {
				...
				if (typeof file == 'string') {
					...
					if (create_source_map) {
						...
						file = {
							...
							code   : file,
							source : file,
							path   : libpath.resolve(__dirname, 'client.js'),
				...
				if (create_source_map) {
					...
					let target_line = current_line + (file.start || 0),
					    line_count = Blast.Bound.String.count(file.source, '\n'),
					    i;
					...
					for (i = 0; i < line_count; i++) {
						sourcemap.addMapping({
							...
							generated : {line: target_line + i, column: 0}
						});
					}
				}
			}
	 ...
	 * @return    {Array}
	 */
	Blast.convertCoverage = async function convertCoverage(coverage) {
		...
		for (entry of coverage) {
			...
			for (range of ranges) {
				...
				pos = map.originalPositionFor(range.start);
				...
				if (pos.source != current_file) {
					current_file = pos.source;
					new_ranges = [];

					new_entry = {
						// @TODO: should be some path?
						url    : pos.source,
						text   : map.sourceContentFor(pos.source),
						ranges : new_ranges,
					};

					source_lines = new_entry.text.slice('\n');
					new_coverage.push(new_entry);
				}

				let start = getAbsoluteIndex(source_lines, pos),
				    end   = getAbsoluteIndex(source_lines, map.originalPositionFor(range.end));
	 ...
	 * @return    {Object}
	 */
	function getAbsoluteIndex(source_lines, position) {
	...
	function convertIndexRangesToLines(ranges, source, map) {
		...
		// First split the source into lines
		for (i = 0; i < lines.length; i++) {
			line = lines[i];
		...
		for (i = 0; i < ranges.length; i++) {
			...
			if (map) {
				...
				if (position_start.source && position_end.source && position_start.source != position_end.source) {
					...
					// We can safely pus the new range for the first file
