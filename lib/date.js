...
defProto(function startOf(unit) {
	...
	if (unit === 'quarter') {
		this.setMonth(~~(this.getMonth() / 3) * 3);
	}
...
/**
 * Calendar-relative strings (for relativeToCalendar)
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.9.7
 * @version  0.9.7
 ...
 * @return   {string}
 */
defProto('calendar_relative_settings', {
	today          : 'today',
	tomorrow       : 'tomorrow',
	yesterday      : 'yesterday',
	day            : 'a day',
	...
	ago            : 'ago',
	in_prefix      : 'in ',
	week_threshold : 3,   // Minimum days before "last/next week" kicks in
});
...
/**
 * Return a time ago string (relative to now)
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.6.0
 * @version  0.9.7
...
defProto(function timeAgo(settings) {
	return this.relativeTo(Date.now(), settings);
});

/**
 * Return a relative time string compared to another date
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.9.7
 * @version  0.9.7
 *
 * @param    {Date}     reference   The date to compare to (defaults to now)
 * @param    {Object}   settings
 *
 * @return   {string}
 */
defProto(function relativeTo(reference, settings) {

	if (reference == null) {
		reference = Date.now();
	} else if (typeof reference == 'object' && !(reference instanceof Date)) {
		// If first arg is settings object, use now as reference
		settings = reference;
		reference = Date.now();
	}

	let reference_time = typeof reference === 'number' ? reference : reference.getTime();
	let difference = reference_time - this.getTime();
...
/**
 * Convert a day difference to a calendar-relative string.
 * This is the core logic used by relativeToCalendar().
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.9.7
 * @version  0.9.7
 *
 * @param    {number}   days              Day difference (positive = future, negative = past)
 * @param    {boolean}  crossed_week      Whether a week boundary was crossed
 * @param    {Object}   settings          Optional settings override
 *
 * @return   {string}
 */
defStat(function daysToCalendarRelative(days, crossed_week, settings) {

	let base_settings = Bound.Date.prototype.calendar_relative_settings;
	...
	if (days === 0) return settings.today;
	if (days === 1) return settings.tomorrow;
	if (days === -1) return settings.yesterday;

	let prefix = future ? settings.in_prefix : '';
	let suffix = future ? '' : ' ' + settings.ago;

	// Check if we should use "last week" / "next week"
	// Only if: crossed week boundary AND at least threshold days AND less than 7 days
	// (At 7+ days, "a week ago" / "2 weeks ago" is more precise)
	let week_threshold = settings.week_threshold || 3;

	if (crossed_week && abs_days >= week_threshold && abs_days < 7) {
		return future ? settings.next_week : settings.last_week;
	}

	// Less than 7 days: use "X days"
	if (abs_days < 7) {
		return prefix + numberString(settings.days, abs_days) + suffix;
	}

	// Calculate weeks
	let weeks = Math.floor(abs_days / 7);

	// 7-13 days: "a week ago" / "in a week"
	if (abs_days < 14) {
		return prefix + settings.week + suffix;
	}

	// 14-27 days: use weeks
	if (abs_days < 28) {
		return prefix + numberString(settings.weeks, weeks) + suffix;
	}

	// Calculate months
	let months = Math.round(abs_days / 30);

	// ~1 month (28-45 days)
	if (abs_days < 45) {
		return future ? settings.next_month : settings.last_month;
	}

	// Less than ~1 year: use months
	if (abs_days < 365) {
		return prefix + numberString(settings.months, months) + suffix;
	}

	// Calculate years
	let years = Math.round(abs_days / 365);

	// 1 year
	if (years === 1) {
		return future ? settings.next_year : settings.last_year;
	}

	// Multiple years
	return prefix + numberString(settings.years, years) + suffix;
});

/**
 * Return a calendar-relative string compared to another date.
 * Unlike relativeTo(), this uses calendar days (not duration) and
 * supports today/tomorrow/yesterday/last week etc.
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.9.7
 * @version  0.9.7
 ...
 * @return   {string}
 */
defProto(function relativeToCalendar(reference, settings) {
	...
	// Get the day difference using calendar days (startOfUnit = true)
	let days = Math.round(Bound.Date.difference('day', reference, this, true));

	// Check if we crossed a week boundary
	// Get day of week (0 = Sunday, 1 = Monday, etc.)
	let ref_day_of_week = reference.getDay();
	let this_day_of_week = this.getDay();

	// Determine if a week boundary was crossed
	// For past dates: if we went back past the start of the reference's week
	// For future dates: if we went forward past the end of the reference's week
	let crossed_week = false;

	if (days < 0) {
		// Past: check if we crossed back past the week start (Monday = 1, or Sunday = 0)
		// Days to start of week (assuming Monday is start)
		let days_since_week_start = ref_day_of_week === 0 ? 6 : ref_day_of_week - 1;
		crossed_week = Math.abs(days) > days_since_week_start;
	} else if (days > 0) {
		// Future: check if we crossed forward past the week end
		let days_until_week_end = ref_day_of_week === 0 ? 0 : 7 - ref_day_of_week;
		crossed_week = days > days_until_week_end;
	}

	return Bound.Date.daysToCalendarRelative(days, crossed_week, settings);
});