...
// Maximum number of filter events to remember (prevents memory leak)
Informer.setStatic('max_filter_seen', 1000);
...
Informer.setMethod(function queryListeners(type, mark_as_seen) {
	...
	if (filter) {
		...
		if (mark_as_seen) {
			let filter_seen = this.filterSeen;
			filter_seen.push(filter);

			// Enforce maximum size limit to prevent memory leaks
			let max_size = this.constructor.max_filter_seen || Informer.max_filter_seen || 1000;

			if (filter_seen.length > max_size) {
				// Remove oldest entries (first 10% or at least 1)
				let remove_count = Math.max(1, Math.floor(max_size * 0.1));
				filter_seen.splice(0, remove_count);
			}
		}
