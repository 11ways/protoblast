module.exports = function BlastInformer(Blast, Collection) {
	...
	Blast.defineValue(Informer.prototype, 'emit', function emit(type) {
			...
			tasks[tasks.length] = function doListener(next) {
				...
				switch (argLength) {
					...
					case 1:
						...
						break;
					case 2:
						listeners[i][0].call(context, args[1]);
						break;
					case 3:
						listeners[i][0].call(context, args[1], args[2]);
						break;
		...
		Blast.Bound.Function.series(tasks, function seriesDone(err) {
	 ...
	 */
	Blast.defineValue(Informer.prototype, 'emitOnce', function emitOnce(type, data) {
		if (!this.hasBeenSeen(type)) {
			this.emit(type, data);
		}
	});
