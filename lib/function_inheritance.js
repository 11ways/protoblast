module.exports = function BlastInheritance(Blast, Collection) {
	...
	Blast.defineStatic('Function', 'inherits', function inherits(_parent, _namespace, _newConstructor) {
		...
		if (namespace && !newConstructor.namespace) {
		...
		if (Array.isArray(names)) {
			...
			for (i = 0; i < names.length; i++) {
				...
				if (namespace == null && path.indexOf('.') > -1) {
					temp = path.split('.');
					temp.pop();
					namespace = temp.join('.');
				}
				...
				if (typeof Obj.path(Blast.Classes, path) !== 'function' && typeof Obj.path(Blast.Globals, path) !== 'function') {
					Blast.once({type: 'extended', descendant: path}, function whenClassAvailable() {
		...
		if (Blast.emit) {
			Blast.nextTick(function() {
				Blast.emit({type: 'extended', ancestor: parentConstructor.name, descendant: newConstructor.name}, parentConstructor, newConstructor, null);
			});
		}
