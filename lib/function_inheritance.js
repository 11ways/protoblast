const finished_constitutors = new WeakMap(),
      ...
      POISONED = Symbol('poisoned'),
      FALLBACK_CONSTRUCTOR = Symbol('fallback_constructor'),
...
Blast.defineStatic('Function', function inherits(_parent, _namespace, _newConstructor, _do_constitutors) {
	...
	if (typeof newConstructor == 'string') {
		...
		try {
			newConstructor = Fn.create(newConstructor, function constructorShim() {
				constructorShim.super.apply(this, arguments);
			});
		} catch (err) {
	...
	let poisoned_by_class_syntax = false,
	    fallback_constructor,
	    newly_poisoned = false;

	if (parentConstructor) {
		poisoned_by_class_syntax = parentConstructor.prototype[POISONED];

		if (poisoned_by_class_syntax == null) {
			poisoned_by_class_syntax = Fn.isNativeClass(parentConstructor);
			newly_poisoned = poisoned_by_class_syntax;
		}

		if (poisoned_by_class_syntax) {
			...
			if (newly_poisoned) {
				replacement_class = class extends parentConstructor {
					constructor() {
						super(...arguments);
						this[FALLBACK_CONSTRUCTOR](...arguments);
					}
				};
			} else {
				replacement_class = class extends parentConstructor {};
	...
	if (poisoned_by_class_syntax) {
		newConstructor.setMethod(FALLBACK_CONSTRUCTOR, fallback_constructor);
	}
