module.exports = function BlastInheritance(Blast, Collection) {
				...
				if (typeof Obj.path(Blast.Classes, path) !== 'function' && typeof Obj.path(Blast.Globals, path) !== 'function') {
					Blast.once({type: 'extended', descendant: name, namespace: namespace}, function whenClassAvailable() {
						...
						for (i = 0; i < oldProto.waitingProperties.length; i++) {
							...
							setSuperGetters(newConstructor, arr[0], arr[1]);
	...
	Blast.defineStatic('Function', 'setProperty', function setProperty(constructor, _key, _getter, _setter) {
		...
		for (i = 0; i < keys.length; i++) {
			...
			if (target.waitingForClass && target.hasOwnProperty('waitingForClass')) {
				...
				// Set a super reference only for the first given getter/setter
				setSuperGetters(constructor, keys[i], config);
			}
	...
	/**
	 * Set super references for setters/getters
	 *
	 * @author   Jelle De Loecker   <jelle@develry.be>
	 * @since    0.2.0
	 * @version  0.2.0
	 ...
	 * @param    {Object}     config       The new property descriptor
	 */
	function setSuperGetters(constructor, key, config) {

		var super_proto,
		    existing;

		// Make sure there is a getter and setter involved
		if (!config.get) {
			return;
		}

		// Make sure the super prototype is set
		if (!constructor || !constructor.super || !constructor.super.prototype) {
			return;
		} else {
			super_proto = constructor.super.prototype;
		}

		// Now get the existing property descriptor
		existing = Blast.Bound.Object.getPropertyDescriptor(super_proto, key);

		if (!existing) {
			return;
		}

		// If there is an existing getter, set it
		if (existing.get && !config.get.super) {
			Blast.defineValue(config.get, 'super', existing.get);
		}

		// Do the same for setter, if there is one
		if (existing.set && config.set && !config.set.super) {
			Blast.defineValue(config.set, 'super', existing.set);
		} else if (existing.set && !config.set && config.set !== null) {
			// If we're overwriting the property, but haven't supplied a setter
			// and the parent DOES have one, inherit that
			// Unless the new setter is explicitly set to null
			config.set = existing.set;
		}
	}
