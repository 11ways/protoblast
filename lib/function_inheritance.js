module.exports = function BlastInheritance(Blast, Collection) {
	...
	Blast.defineStatic('Function', 'inherits', function inherits(_parent, _namespace, _newConstructor, _do_constitutors) {
			...
			for (i = 0; i < names.length; i++) {
				...
				if (typeof Obj.path(Blast.Classes, path) !== 'function' && typeof Obj.path(Blast.Globals, path) !== 'function') {
					Blast.once({type: 'extended', descendant: name, namespace: parent_namespace}, function whenClassAvailable() {
						...
						console.log('Class', name, 'has become available');
					...
					Blast.loaded(function doConstitutorsWhenLoaded() {
						...
						console.log('Parent of', newConstructor.name, 'is', newConstructor.super)
		...
		if (Blast.emit) {
			console.log('Created', newConstructor.name);
		...
		var delayed = Blast.loaded(function doConstitutorsWhenLoaded(already_loaded) {

			if (already_loaded) {
				console.log('Redirecting', newConstructor.name);
				return Blast.setImmediate(doConstitutorsWhenLoaded);
			}

			console.log('Doing constitutors of', newConstructor.name, 'delayed', delayed);
	...
	function doConstitutors(constructor) {
		...
		console.log('Doing constitutors for children of', constructor.name);
	 ...
	 */
	Blast.defineStatic('Function', 'constitute', function constitute(constructor, task, from_parent) {
		...
		if (Blast.loaded()) {
			console.log('Already loaded, executing', task.name, 'for', constructor.name)

			if (has_constituted.get(constructor)) {
				console.log('Already constitutedç', constructor.name)
				doConstructorTask(constructor, task);
				return;
			}

			Blast.setImmediate(function doConstitutor() {
				console.log(' --', task.name, 'for', constructor.name)
				doConstructorTask(constructor, task);
			});
		}
		...
		if (constructor.prototype.waitingConstitute) {
			old_proto = constructor.prototype;
			constructor.prototype.waitingConstitute.push(task);

			Blast.loaded(function onceLoaded(already_loaded) {

				var i;

				console.log('Delaying constitutors for', constructor.name, old_proto.waitingConstitute)

				// If it's still waiting for the parent, do nothing yet
				if (constructor.prototype.waitingForClass) {
					return;
				}

				for (i = 0; i < constructor.constitutors.length; i++) {
					doTask(constructor.constitutors[i]);
				}
			});
		...
		// Execute the task on this constructor as soon as possible,
		// but after all the scripts have finished
		Blast.loaded(function onceLoaded() {
			doTask(task);
		});
