module.exports = function BlastInheritance(Blast, Collection) {
	...
	/**
	 * Inherit from a native class
	 *
	 * @author   Jelle De Loecker   <jelle@develry.be>
	 * @since    0.7.1
	 * @version  0.7.1
	 *
	 * @param    {Function}   native_class
	 * @param    {Function}   fnc
	 * @param    {String}     class_name
	 */
	function inheritFromNativeClass(native_class, fnc, class_name) {

		var result;

		eval(`result = class ${class_name} extends native_class {
			constructor() {
				super(...arguments);
		...
		result.prototype[CONSTRUCTOR] = fnc;
	...
	Blast.defineStatic('Function', function inherits(_parent, _namespace, _newConstructor, _do_constitutors) {
		...
		if (parentConstructor && Blast.Bound.Function.isNativeClass(parentConstructor)) {

			newConstructor = inheritFromNativeClass(parentConstructor, newConstructor, new_constructor_name);
