 ...
 * @type     {number}
 */
Request.setProperty('max_response_size', 100 * 1024 * 1024);
 ...
 * @type     {number}
 */
Request.setProperty('max_redirects', 20);

/**
 * Maximum number of retries for ECONNRESET errors (default: 3)
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.9.6
 * @version  0.9.6
 ...
 * @type     {number}
 */
Request.setProperty('max_retries', 3);
...
Request.setMethod(function start() {
	...
	// Do not cache stream requests:
	// the stream will have emptied on the second attempt!
	if (this.get_stream) {
		this.cache = false;
	}

	let try_cache = this.cache ?? this.blast_cache ?? this.method == 'GET';
	...
	if (try_cache) {
		...
		let key_params = [
			this.url.href,
			this.headers,
			this.download_if_inline,
		];

		if (this.method != 'GET') {
			key_params.push(this.body);
		}

		key = Classes.Object.checksum(key_params);
...
Request.setMethod(function _resolveWithCache(cached_pledge, key) {
	...
	let result = new Classes.Pledge.Swift();

	this.cached_request = ori_req;
	result.request = this;

	Classes.Pledge.Swift.done(cached_pledge, (err, res) => {
 ...
 * @return   {Pledge}
 */
Blast.fetch = function fetch(url, options, callback) {
	...
	if (!options) {
		options = {};
	} else if (typeof options == 'string' || Blast.Classes.RURL.isUrl(options)) {
		options = {url: options};
	}
