 ...
 * - Shows decimal only if meaningful (non-zero)
 * - Rounds to specified precision (half away from zero)
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.9.7
 * @version  0.9.7
 *
 * @param    {number}   max_decimals   Maximum decimal places to show (default: 1)
 *
 * @return   {string}
 *
 * @example
 *   (7.5).toSmartDecimal()   // "7.5"
 *   (8.0).toSmartDecimal()   // "8"
 *   (7.25).toSmartDecimal()  // "7.3" (rounds to 1 decimal)
 *   (7.25).toSmartDecimal(2) // "7.25"
 *   (7.00).toSmartDecimal(2) // "7"
 */
defProto(function toSmartDecimal(max_decimals) {
	...
	let factor = Math.pow(10, max_decimals);
	
	// Round half away from zero (more intuitive for negative numbers)
	// Math.round rounds towards positive infinity, so -7.25 becomes -7.2
	// We want -7.25 to become -7.3 (away from zero)
	let sign = this < 0 ? -1 : 1;
	let rounded = sign * Math.round(Math.abs(this) * factor) / factor;

	// Check if the rounded value is effectively an integer
	if (rounded % 1 === 0) {
		return String(rounded);
	}

	return rounded.toFixed(max_decimals);
});