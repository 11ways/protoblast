 ...
 * @return   {string}
 */
AbstractDateTime.setMethod(function toJSON() {
...
/**
 * Custom toPrimitive behaviour (like Date)
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.8.14
 * @version  0.8.14
 ...
 * @return   {Mixed}
 */
AbstractDateTime.setMethod(Symbol.toPrimitive, function toPrimitive(hint) {

	if (!hint || hint == 'string' || hint == 'default') {
		return this.toString();
	}
 ...
 * Uses the same logic as Date.prototype.relativeTo()
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.9.7
 * @version  0.9.7
 *
 * @param    {AbstractDateTime|Date}   reference   The date to compare to (defaults to now)
 * @param    {Object}                  settings
 *
 * @return   {string}
 */
AbstractDateTime.setMethod(function relativeTo(reference, settings) {

	if (reference == null) {
		reference = new Date();
	} else if (typeof reference == 'object' && !(reference instanceof Date) && !(reference instanceof AbstractDateTime)) {
		// If first arg is settings object, use now as reference
		settings = reference;
		reference = new Date();
	}

	// Convert to native Date for the calculation
	let this_date = this.getNativeDate();
	let ref_date = reference instanceof AbstractDateTime ? reference.getNativeDate() : reference;

	// Use the prototype method directly (Bound functions expect this as first arg, not via .call)
	return this_date.relativeTo(ref_date, settings);
});

/**
 * Return a time ago string (relative to now).
 * Uses the same logic as Date.prototype.timeAgo()
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.9.7
 * @version  0.9.7
 ...
 * @return   {string}
 */
AbstractDateTime.setMethod(function timeAgo(settings) {
	return this.relativeTo(new Date(), settings);
});

/**
 * Return a calendar-relative string compared to another date.
 * Uses calendar days (not duration) and supports today/tomorrow/yesterday/last week etc.
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.9.7
 * @version  0.9.7
 ...
 * @return   {string}
 */
AbstractDateTime.setMethod(function relativeToCalendar(reference, settings) {
	...
	// Use the prototype method directly (Bound functions expect this as first arg, not via .call)
	return this_date.relativeToCalendar(ref_date, settings);
});
 ...
 * @return   {string}
 */
AbstractDateTime.setMethod(Symbol.for('janeway_arg_left'), function janewayClassIdentifier() {
 ...
 * @return   {string}
 */
AbstractDateTime.setMethod(Symbol.for('janeway_arg_right'), function janewayInstanceInfo() {
	return this.toString();
});