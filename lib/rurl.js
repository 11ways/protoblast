...
RURL.setStatic(function extractProtocol(address) {
	...
	if (rest && isFinite(rest) && protocol != 'tel:') {
		rest = protocol;
		protocol = '';
		adjusted = true;
	}
 ...
 * @return   {string}
 */
RURL.setStatic(function encodePathSegment(val) {
...
RURL.setStatic(function parseQueryValues(input, options) {
	...
	options = Object.assign({}, RURL.default_query_parse_options, options);
...
RURL.setProperty(function pathname() {
	...
	if (pathname) {
		...
		for (i = 0; i < pieces.length; i++) {
			pieces[i] = RURL.encodePathSegment(pieces[i]);
		}
...
/**
 * See if the current path matches the given one
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.8.7
 * @version  0.8.7
 *
 * @param    {RURL|String}   other
 *
 * @return   {boolean}
 */
RURL.setMethod(function matchesPath(other) {

	if (!other) {
		return;
	}

	other = RURL.parse(other);

	let our_segments = this.segments,
	    their_segments = other.segments;
	
	if (our_segments.length != their_segments.length) {
		return false;
	}

	for (let i = 0; i < our_segments.length; i++) {

		if (our_segments[i] != their_segments[i]) {
			return false;
		}
	}
