...
Blast.definePrototype('Error', function toDry() {
	...
	if (this.constructor.name != 'Error' && !this.constructor.super) {

		if (typeof values.message == 'string' && values.message.indexOf(this.constructor.name) == -1) {
			result.value.message = this.constructor.name + ': ' + values.message;
		}
