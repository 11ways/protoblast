module.exports = function BlastURL(Blast, Collection) {
	...
	if (Blast.isNode && !Blast.isNWWindow) {
		...
		try {
			// Pre-node v10
			Object.defineProperty(Blast.Classes.URL, 'prototype', {
				value: Object.create(liburl.Url.prototype)
			});
		} catch (err) {
			// From node v10 & onwards
	 ...
	 * @return   {String}
	 */
	Blast.defineStatic('URL', 'encodeUriQuery', function encodeUriQuery(val, pct_encode_spaces) {
	 ...
	 * @return   {String}
	 */
	Blast.defineStatic('URL', 'encodeUriSegment', function encodeUriSegment(val) {
		return Collection.URL.encodeUriQuery(val, true)
			.replace(/%5b/gi, '[')
			.replace(/%5d/gi, ']');
	});
	...
	Blast.defineStatic('URL', 'encodeQuery', function encodeQuery(obj, sep, eq, options) {
		...
		if (!options.encodeURIComponent) {
			options.encodeURIComponent = Collection.URL.encodeUriSegment;
		}
