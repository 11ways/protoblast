module.exports = function BlastObject(Blast, Collection, Bound, Obj) {
	...
	Blast.defineStatic('Object', 'exists', function exists(obj, path) {
		...
		if (path.length) {
			obj = Bound.Object.path(obj, path);
		}
	...
	Blast.defineStatic('Object', 'checksum', function checksum(obj, sort_arrays, seen) {
		...
		if (Array.isArray(obj)) {
			...
			result = 'A';
		} else if (obj.constructor && obj.constructor.name == 'Buffer') {
		...
		for (i = 0; i < names.length; i++) {
			...
			if (val) {
				...
				if (Bound.Object.isPrimitiveObject(val)) {
					val = ''+val;
	...
	Blast.defineStatic('Object', 'alike', function alike(a, b, seen) {
		...
		if (Bound.Object.size(a, false) != Bound.Object.size(b, false)) {
			return false;
		}
		...
		if (a.constructor && a.constructor.name == 'Buffer'
			&& b.constructor && b.constructor.name == 'Buffer') {
			return a.equals(b);
		}
