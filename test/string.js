...
<strong>d</strong
	...
	describe('#replaceAll(needle, replacement) - Shim', function() {
		...
		it('should be able to replace with a replacement containing the needle', function() {
			...
			const replaceAll = Blast.Shims['String#replaceAll'];

			let input = 'change all as to double as';
			let result = replaceAll.call(input, 'a', 'aa');

			assert.strictEqual(result, 'chaange aall aas to double aas');
		});
		...
		it('should replace all the occurences of needle', function() {
			...
			var input = 'change all as to es',
			    result = replaceAll.call(input, 'a', 'e');
			...
			assert.strictEqual(replaceAll.call(str, /a/g, 'z'), str.replace(/a/g, 'z'));

			// replaceAll, with a string, replaces all
			assert.strictEqual(replaceAll.call(str, 'a', 'z'), 'zzbc');

			let count = 0;

			let fnc_result = replaceAll.call(str, 'a', function(match) {
				return count++;
			});
		...
		it('should not treat strings as regexes', function() {

			const replaceAll = Blast.Shims['String#replaceAll'];

			let input = 'ulysses://x-callback-url/open?id=1 ulysses://x-callback-url/open?id=2 ulysses://x-callback-url/open?id=3';

			let result = replaceAll.call(input, 'ulysses://x-callback-url/open?id=', '_');

			assert.strictEqual(result, '_1 _2 _3');
		});

		it('should be able to replace emptyness', function() {

			const replaceAll = Blast.Shims['String#replaceAll'];

			let result = replaceAll.call('xxx', '', '_');

			assert.strictEqual(result, '_x_x_x_');
		});

		it('should be able to use replacement patterns', function() {

			const replaceAll = Blast.Shims['String#replaceAll'];

			assert.strictEqual(replaceAll.call('121314', '1', '$$'), '$2$3$4');
			assert.strictEqual(replaceAll.call('121314', '1', '$&'), '121314');
			assert.strictEqual(replaceAll.call('121314', '1', '$`'), '212312134');
			assert.strictEqual(replaceAll.call('121314', '1', '$\''), '213142314344');
		});

		it('should accept a function as a replacement', function() {

			const replaceAll = Blast.Shims['String#replaceAll'];

			let result = replaceAll.call('aba', 'b', (search, i, string) => {
				assert.strictEqual(search, 'b', '`search` is `b`');
				assert.strictEqual(i, 1, '`i` is 1');
				assert.strictEqual(string, 'aba', '`string` is `aba`');
				return 'c';
			});

			assert.strictEqual(result, 'aca');
		});
	});
